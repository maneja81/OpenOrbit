...
import { testContactsConnection } from "./googleContactsConnector";

const fetchMock = vi.fn();

describe("testContactsConnection", () => {
  beforeEach(() => {
  ...
  it("falls back to the display name when no email address is present", async () => {
    fetchMock.mockResolvedValue({
      ok: true,
  ...
  it("uses a refreshed token when the stored credentials are expired", async () => {
    ...
    await testContactsConnection(expiredCredentials);
